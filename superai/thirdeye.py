...
def criticize_outcome(task_id: str, outcome: dict, evidence: list[dict] = None) -> dict:
    """Critically analyze a task outcome.
    
    Questions:
    - Did the task actually finish?
    - Is there evidence?
    - Does evidence support the claim?
    - Does result match the request?
    - Were there side effects?
    - Was more done than necessary?
    """
    findings = []
    recommendations = []
    ...
    ev_count = outcome.get("evidence_count", 0) if "evidence_count" in outcome else len(evidence or [])
    
    # Question 1: Did the task actually finish?
    if state in ("PLANNED", "RUNNING"):
        findings.append({
            "check": "outcome_complete",
            "severity": "CRITICAL",
            "msg": f"Task state is '{state}' — not completed",
            "evidence": {"state": state},
        })
    
    # Question 2: Is there evidence?
    if ev_count == 0:
        findings.append({
            "check": "outcome_evidence",
            "severity": "CRITICAL" if success else "WARNING",
            "msg": "No evidence collected for outcome",
            "evidence": {"evidence_count": 0},
        })
        if success:
            recommendations.append({
                "type": "verification",
                "msg": "Cannot verify success without evidence. Mark as UNVERIFIED.",
            })
    
    # Question 3: Does evidence support the claim?
    if success and evidence:
        ...
        if failed_evidence:
            findings.append({
                ...
                "severity": "CRITICAL",
                "msg": f"Success claimed but {len(failed_evidence)} evidence items failed",
                "evidence": {"failed": [e.get("type") for e in failed_evidence]},
            })
        
        non_measured = [e for e in evidence if e.get("kind") != "MEASURED"]
        if non_measured:
            findings.append({
                "check": "outcome_quality",
                "severity": "WARNING",
                "msg": f"{len(non_measured)} evidence items not MEASURED",
                "evidence": {"non_measured": [e.get("kind") for e in non_measured]},
            })
    
    # Question 4: Does result match the request?
    task_action = outcome.get("action", "")
    if task_action and success:
        # Check if evidence types match the action
        ev_types = set(e.get("type") for e in (evidence or []))
        if "file_exists" in ev_types and "write" in task_action.lower():
            # File write should have content evidence too
            if "file_content" not in ev_types and "syntax" not in ev_types:
                findings.append({
                    "check": "outcome_match",
                    "severity": "INFO",
                    "msg": "File write has existence evidence but no content verification",
                    "evidence": {"types": list(ev_types)},
                })
    
    # Question 5: Were there side effects?
    if evidence:
        for ev in evidence:
            if ev.get("type") == "git_state" and not ev.get("clean", True):
                findings.append({
                    "check": "side_effects",
                    "severity": "INFO",
                    "msg": "Working tree not clean after task",
                    "evidence": {"modified_files": ev.get("modified_files", 0)},
                })
    
    # Question 6: Was more done than necessary?
    if ev_count > 10:
        findings.append({
            "check": "outcome_efficiency",
            "severity": "INFO",
            "msg": f"Large evidence set ({ev_count} items) — verify all were necessary",
            "evidence": {"evidence_count": ev_count},
        })
...
def format_outcome_criticism(critique: dict) -> str:
    """Format outcome criticism for chat output."""
    lines = [
        f"OUTCOME VERIFICATION — {critique['overall']}",
        f"Task: {critique['task_id']}",
    ]
